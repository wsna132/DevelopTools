...
//                dontAnimate().//去除图片加载时的淡入淡出效果
     ...
     * @param targetImageView
     */
    private void showImageDemo(Activity context, int imageId, ImageView targetImageView) {
        Glide.with(context).
                load(android.R.mipmap.sym_def_app_icon).
...
//                asBitmap().  如果你仅仅想要显示 Gif 的第一帧，你可以调用 asBitmap() 去保证其作为一个常规的图片显示，即使这个 URL 是一个 Gif
        placeholder(ImageLoadConsts.IMG_LOADING). // 加载中显示
                error(ImageLoadConsts.IMG_LOAD_ERROR). // 加载失败时现实
                dontAnimate().//去除图片加载时的淡入淡出效果
                override(600, 200). // 设置图片尺寸resizes the image to these dimensions (in pixel). does not respect aspect rati   单位为像素
...
//    DiskCacheStrategy.ALL 缓存所有版本的图像（默认行为）
    ...
    private void loadImageCustom(Context context,final ImageView imageView1) {
        SimpleTarget target = new SimpleTarget<Bitmap>() {
            @Override
            public void onResourceReady(Bitmap bitmap, GlideAnimation glideAnimation) {
                // do something with the bitmap
                // for demonstration purposes, let's just set it to an ImageView
                imageView1.setImageBitmap(bitmap);//这样甚至可以赋值给自定义的View
            }
        };
        //这个用来监听图片的加载，加载失败时可以输出报错信息
